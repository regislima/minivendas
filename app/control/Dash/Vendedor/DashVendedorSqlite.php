...
class DashVendedorSqlite extends TPage
{
    ...
    private function getTotalVendasDia()
    {
        ...
        $criteria->add(new TFilter('strftime("%Y-%m-%d", created_at)', '=', date('Y-m-d')));
        
        $repo = new TRepository('Venda');
        $vendasDia = $repo->count($criteria);
    ...
    private function getTotalVendasMes()
    {
        ...
        $criteria = new TCriteria;
        $criteria->add(new TFilter('id_usuario', '=', $idVendedor));
        $criteria->add(new TFilter('strftime("%m", created_at)', '=', date('m')));
        $criteria->add(new TFilter('strftime("%Y", created_at)', '=', date('Y')));
    ...
    private function getTotalValorVendasDia() : float
    {
        ...
        $valorVendasDia = Venda::where('id_usuario', '=', $idVendedor)->where('strftime("%Y-%m-%d", created_at)', '=', date('Y-m-d'))->where('id_forma_pagamento', '=', 1)->sumBy('valor_final') ?? 0;
    ...
    private function getTotalValorVendasMes(int $mes = null) : float
    {
        ...
        $valorVendasMes = Venda::where('id_usuario', '=', $idVendedor)->where('strftime("%Y-%m", created_at)', '=', $data)->where('id_forma_pagamento', '=', 1)->sumBy('valor_final') ?? 0;
